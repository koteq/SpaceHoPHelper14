...
import { info, removeOptions, trace } from './utils.js';
...
export async function setProfile(profileName) {
  ...
  /** @typedef {{ title: string, template: string }} Template */
  /** @typedef {{ type: string, templates: Template[] }} Item */
  /** @typedef {{ name: string, items: Item[] }} SubCategory */
  /** @typedef {{ name: string, subcategories: SubCategory[] }} Category */
  /** @typedef {{ name: string, fields: string[], categories: Category[] }} Section */
  /** @typedef {{ name: string, sections: Section }} Group */
  /** @typedef {{ profile: string, groups: Group[], positions: Record<string, string[]>, departments: string[], departmentsCodes: string[] }} Profile */
  /** @type Profile */
  let profile;
  let profilePath;
  const url = './profiles/';
  ...
  const yamlContent = await (await fetch(profilePath)).text();
  profile = yaml.load(yamlContent);
  ...
  const departmentCodeSelect = document.querySelector(
    '#department-code-field > select'
  );
  ...
  positionSelects.forEach((select) => {
    const undefinedOption = Section(null, {
      id: 1,
      name: profile.positions.undefined,
    });
    ...
    Object.entries(profile.positions).forEach(([department, positions]) => {
      ...
      positions.forEach((position) => {
        const positionOption = Section(departmentGroup, {
          id: nextId,
          name: position,
        });
        nextId += 1;
  ...
  const hideFields = () => {
    formFields.forEach((field) => field.classList.add('d-none'));
    currentFields = [];
  };
  ...
  statementTypeSelect.addEventListener('change', (e) =>
  ...
  document
    ...
    .addEventListener('submit', (e) => {
      ...
      const selectFields = (d) => {
        ...
        obj['spaces'] = ' '.repeat(
          Math.max((31 - obj['stationNumber'].length) / 2, 0)
        );
        trace(`Field data: ${obj['date']}`);
        trace(`Field time: ${obj['time']}`);
        trace(`Field stationNumber: ${obj['stationNumber']}`);
        trace(
          `Field spaces: " " x ${Math.max((31 - obj['stationNumber'].length) / 2, 0)}`
        );
        console.log(obj);
        return obj;
      };
        ...
        category.subcategories.forEach((sub) => {
          ...
          sub.items.forEach((item) => {
            ...
            templates.forEach((templateData) => {
              ...
              const cardTemplate = template.replace(
                /\$\{{2}(\w+)}{2}/g,
                (_, key) => dataObject[key]
              );
          ...
          trace(
            `Adding subcategory ${sub.name} to category ${category.name}...`
          );
