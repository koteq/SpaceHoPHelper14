/**
 * SCEditor SS14 BBCode Plugin
 *
 * See
 * https://unpkg.com/sceditor@3.2.0/src/formats/bbcode.js
 * https://github.com/space-wizards/RobustToolbox/blob/cd0a35f542429bdea0f1b555bc52299502fb5605/Robust.Client/UserInterface/RichTextEntry.cs
 * https://github.com/space-wizards/RobustToolbox/blob/cd0a35f542429bdea0f1b555bc52299502fb5605/Robust.Shared/Utility/FormattedMessage.MarkupParser.cs#L11
 */
import * as libdom from 'https://unpkg.com/sceditor@3.2.0/src/lib/dom.js';
...
(function (sceditor) {
  ...
  sceditor.command.set('size', {
    _dropDown: function (editor, caller, callback) {
      const content = libdom.createElement('div');

      libdom.on(content, 'click', 'a', function (e) {
        callback(libdom.data(this, 'size'));
      ...
      for (let i = 1; i <= 3; i++) {
        libdom.appendChild(
            content,
            libdom.parseHTML(
                `<a class="sceditor-fontsize-option" data-size="${i}" href="#"><font size="${7 - i}">${editor._(`Header ${i}`)}</font></a>`
            )
        );
      }
  ...
  const defaultCommandsOverrides = {
    ...
    bolditalic: {
      // Reuse strikethrough browser command for simplicity,
      // while turning <strike> tag into bold italic with CSS
      exec: 'strikethrough',
      txtExec: ['[bolditalic]', '[/bolditalic]'],
      tooltip: 'Bold italic',
  ...
  const bbcodeHandlers = {
    ...
    italic: {
      ...
      html: '<em>{0}</em>',
      closedBy: ['/bold', '/bolditalic'],
    },

    bolditalic: {
      tags: {
        // parse <strike> tag as [bolditallic]
        strike: null,
      },
      format: '[bolditalic]{0}[/bolditalic]',
      html: '<strike>{0}</strike>',
      closedBy: ['/bold', '/italic'],
    },
  ...
  function BBCodeParser(options) {
    ...
    function tokenizeTag(type, val) {
      ...
      if (
        !name ||
        ((type === TOKEN_OPEN || type === TOKEN_CLOSE) && !bbcodeHandlers[name])
      ) {
        type = TOKEN_CONTENT;
        name = '#';
      }
        ...
        switch (token.type) {
          case TOKEN_OPEN:
            ...
            if (
              ...
              (bbcode.closedBy || hasTag(token.name, TOKEN_CLOSE, toks))
            ) {
              openTags.push(token);
          ...
          case TOKEN_NEWLINE:
            ...
            if (
              ...
              next &&
              closesCurrentTag(
                (next.type === TOKEN_CLOSE ? '/' : '') + next.name
              )
            ) {
              // skip if the next tag is the closing tag for
              // the option tag, i.e. [/*]
              if (
                !(next.type === TOKEN_CLOSE && next.name === currentTag().name)
              ) {
                bbcode = bbcodeHandlers[currentTag().name];
      ...
      for (i = 0; i < children.length; i++) {
        ...
        if (insideInline && !isInline(token)) {
          ...
          parentParentChildren =
            parents.length > 1 ? parents[parents.length - 2].children : rootArr;
    ...
    function convertToBBCode(toks) {
      ...
      while (toks.length > 0) {
        ...
        breakEnd =
          (isBlock && base.opts.breakEndBlock && bbcode.breakEnd !== false) ||
        ...
        if (!bbcode && token.type === TOKEN_OPEN) {
          ...
          if (token.attrs) {
            if (token.attrs.defaultattr) {
              ret +=
                '=' + quote(token.attrs.defaultattr, quoteType, 'defaultattr');
            ...
            for (attr in token.attrs) {
              if (token.attrs.hasOwnProperty(attr)) {
                ret +=
                  ' ' + attr + '=' + quote(token.attrs[attr], quoteType, attr);
              }
            }
          }
          ret += ']';
  ...
  function _normaliseColour(colorStr) {
    ...
    // rgb(n,n,n);
    if (
      (match = colorStr.match(/rgb\((\d{1,3}),\s*?(\d{1,3}),\s*?(\d{1,3})\)/i))
    ) {
  ...
  function bbcodeFormat() {
    ...
    function elementToBbcode(element, hasCodeParent) {
      const toBBCode = function (node, hasCodeParent, vChildren) {
        let ret = '';
    ...
    function toHtml(asFragment, source, legacyAsFragment) {
      ...
      const toHTML =
        asFragment || legacyAsFragment ? parser.toHTMLFragment : parser.toHTML;
